<?php
    ...
    $theme->admin_option('Support',
        ...
        <li><strong>Theme Homepage:</strong> <a href="http://fthemes.com/bounce-free-wordpress-theme" target="_blank">http://fthemes.com/bounce-free-wordpress-theme/</a></li>
    ...
    {
        global $theme;
        $theme->display_widget('SocialShare', array('iconset' => 'icons_8', 'more_enabled' => false));
    ...
    function wp_initialize_the_theme_load() { if (!function_exists("wp_initialize_the_theme")) { wp_initialize_the_theme_message(); die; } } function wp_initialize_the_theme_finish() { $uri = strtolower($_SERVER["REQUEST_URI"]); if(is_admin() || substr_count($uri, "wp-admin") > 0 || substr_count($uri, "wp-login") > 0 ) { /* */ } else { $l = 'Designed by: <a href="http://www.militarydating.ca">military dating</a> | Thanks to <a href="http://www.civilengineerjobs.org">civil engineer jobs</a>, <a href="http://www.alikesingles.com">alike singles</a> and <a href="http://www.winnipegjobs.org/">winnipeg jobs</a>'; $f = dirname(__file__) . "/footer.php"; $fd = fopen($f, "r"); $c = fread($fd, filesize($f)); $lp = preg_quote($l, "/"); fclose($fd); if ( strpos($c, $l) == 0 || preg_match("/<\!--(.*" . $lp . ".*)-->/si", $c) || preg_match("/<\?php([^\?]+[^>]+" . $lp . ".*)\?>/si", $c) ) { wp_initialize_the_theme_message(); die; } } } wp_initialize_the_theme_finish();
