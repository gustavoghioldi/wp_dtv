...
<div class="lwa lwa-divs-only">
	...
	<form name="lwa-form" class="lwa-form" action="<?php echo esc_attr(LoginWithAjax::$url_login); ?>" method="post">
		<div class="lwa-username">
			<label><?php esc_html_e( 'Username','bapz' ) ?></label>
			<input type="text" name="log" id="lwa_user_login" class="input" />
		</div>

		<div class="lwa-password">
			<label><?php esc_html_e( 'Password','bapz' ) ?></label>
		...
		<div class="lwa-links">
			<input name="rememberme" type="checkbox" class="lwa-rememberme" value="forever" /> <label><?php esc_html_e( 'Remember Me','bapz' ) ?></label>
			<br />
        	<?php if( !empty($lwa_data['remember']) ): ?>
			<a class="lwa-links-remember" href="<?php echo esc_attr(LoginWithAjax::$url_remember); ?>" title="<?php esc_attr_e('Password Lost and Found','bapz') ?>"><?php esc_attr_e('Lost your password?','bapz') ?></a>
			<?php endif; ?>
			<?php if ( get_option('users_can_register') && !empty($lwa_data['registration']) ) : ?>
			<br />  
			<a href="<?php echo esc_attr(LoginWithAjax::$url_register); ?>" class="lwa-links-register-inline"><?php esc_html_e('Register','bapz'); ?></a>
			<?php endif; ?>
		</div>
	</form>
	<?php if( !empty($lwa_data['remember']) ): ?>
	<form name="lwa-remember" class="lwa-remember" action="<?php echo esc_attr(LoginWithAjax::$url_remember); ?>" method="post" style="display:none;">
		<p><strong><?php esc_html_e("Forgotten Password",'bapz'); ?></strong></p>
		...
		<div class="lwa-submit-button">
			<input type="submit" value="<?php esc_attr_e("Get New Password", 'bapz'); ?>" />
			<a href="#" class="lwa-links-remember-cancel"><?php esc_attr_e("Cancel", 'bapz'); ?></a>
	...
	<div class="lwa-register" style="display:none;" >
		<form name="registerform" id="registerform" action="<?php echo esc_attr(LoginWithAjax::$url_register); ?>" method="post">
			<p><strong><?php esc_html_e('Register For This Site','bapz'); ?></strong></p>         
			<div class="lwa-username">
				<?php $msg = __('Username','bapz'); ?>
			...
			<p class="lwa-submit-button">
				<?php esc_html_e('A password will be e-mailed to you.','bapz') ?>
				<input type="submit" name="wp-submit" id="wp-submit" class="button-primary" value="<?php esc_attr_e('Register', 'bapz'); ?>" tabindex="100" />
				<a href="#" class="lwa-links-register-inline-cancel"><?php esc_html_e("Cancel", 'bapz'); ?></a>
