...
function siteorigin_panels_black_studio_tinymce_admin_enqueue($page) {
	...
	if ( ( $screen->base == 'post' && in_array( $screen->id, siteorigin_panels_setting('post-types') ) ) || $screen->base == 'appearance_page_so_panels_home_page') {
		...
		if( version_compare($black_studio_tinymce_widget_version, '1.3.3', '<=') ) {
			...
			wp_enqueue_script( 'black-studio-tinymce-widget-siteorigin-panels', CAPS_PANELS_DIR.'widgets/compat/black-studio-tinymce/black-studio-tinymce-widget-siteorigin-panels.old.min.js', array( 'jquery' ), SITEORIGIN_PANELS_VERSION );
		}
		else {
			// Use the new compatibility file
			wp_enqueue_script( 'black-studio-tinymce-widget-siteorigin-panels', CAPS_PANELS_DIR.'widgets/compat/black-studio-tinymce/black-studio-tinymce-widget-siteorigin-panels.min.js', array( 'jquery' ), SITEORIGIN_PANELS_VERSION );
		}

		wp_enqueue_style('black-studio-tinymce-widget-siteorigin-panels', CAPS_PANELS_DIR.'widgets/compat/black-studio-tinymce/black-studio-tinymce-widget-siteorigin-panels.css', array(), SITEORIGIN_PANELS_VERSION);
		...
		if(version_compare($black_studio_tinymce_widget_version, '1.2.0', '<=')) {
			...
			wp_enqueue_script('black-studio-tinymce-widget', CAPS_PANELS_DIR.'widgets/compat/black-studio-tinymce/black-studio-tinymce-widget.min.js', array('jquery'), SITEORIGIN_PANELS_VERSION);
		}
	}
}
