...
<div class="row">
	...
	<div class="<?php echo eo_get_venue()? 'col-lg-6 col-md-6 col-sm-12':'col-lg-12 col-md-12 col-sm-12';?> col-xs-12">
	...
	<ul class="event-meta-list">
		...
		<?php if( !eo_reoccurs() ){ ?>
				<!-- Single event -->
				<li class="start"><span><?php _e('Start', 'eventorganiser') ;?>:</span> <?php eo_the_start($date_format); ?> </li>
				<?php
		 } ?>

		<?php if( eo_get_venue() ){ ?>
			<li class="venue"><span><?php _e('Venue','eventorganiser'); ?>:</span> <a href="<?php eo_venue_link(); ?>"> <?php eo_venue_name(); ?></a></li>
		<?php } ?>


		<?php if( get_the_terms(get_the_ID(),'event-category') ){ ?>
			<li class="categories"><span><?php _e('Categories','eventorganiser'); ?>:</span> <?php echo get_the_term_list( get_the_ID(),'event-category', '', ', ', '' ); ?></li>
		<?php } ?>

	
		<?php if( get_the_terms(get_the_ID(),'event-tag') && !is_wp_error( get_the_terms(get_the_ID(),'event-tag') ) ){ ?>
			<li class="tagcloud"><?php echo get_the_term_list( get_the_ID(),'event-tag', '', '', '' ); ?></li>
		<?php } ?>
