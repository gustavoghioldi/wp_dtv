...
class Shortcodes_Ultimate {
	...
	function __construct() {
		...
		add_action( 'init', array( __CLASS__, 'activation' ) );
		//add_action( 'init', array( __CLASS__, 'deactivation' ) );
	}
	...
	public static function init() {
		...
		/*$admin->add_menu( array(
				'page_title'  => __( 'Shortcodes Ultimate', 'su' ) . ' &lsaquo; ' . __( 'Shortcodes Ultimate', 'su' ),
				'menu_title'  =>  __( 'Shortcodes Ultimate', 'su' ) ,
				'capability'  => 'manage_options',
				'slug'        => 'shortcodes-ultimate',
				'icon_url'    => caps_su_url( 'assets/images/icon.png', SU_PLUGIN_FILE ),
				'position'    => '80.11',
				'options'     => array(
					array(
						'type' => 'opentab',
					...
					array(
						...
						'desc'    => __( 'Disable this option if you have some problems with other plugins or content formatting', 'su' ) . '<br /><a href="#" target="_blank">' . __( 'Documentation article', 'su' ) . '</a>',
						'default' => 'on',
						'label'   => __( 'Enabled', 'su' )
					),
					array(
		...
		/*$admin->add_submenu( array(
				'parent_slug' => 'shortcodes-ultimate',
				'page_title'  => __( 'Shortcode Examples', 'su' ) . ' &lsaquo; ' . __( 'Shortcodes Ultimate', 'su' ),
				'menu_title'  => apply_filters( 'su/menu/examples', __( 'Shortcode Examples', 'su' ) ),
	...
	public static function actions_links( $links ) {
		$links[] = '<a href="' . admin_url( 'themes.php?page=shortcodes-ultimate-examples' ) . '">' . __( 'Examples', 'su' ) . '</a>';
		$links[] = '<a href="' . admin_url( 'themes.php?page=shortcodes-ultimate' ) . '#tab-0">' . __( 'Where to start?', 'su' ) . '</a>';
		return $links;
	}
	...
	public static function meta_links( $links, $file ) {
		...
		if ( $file === plugin_basename( SU_PLUGIN_FILE ) ) {
			unset( $links[2] );
			$links[] = '<a href="#" target="_blank">' . __( 'Project homepage', 'su' ) . '</a>';
