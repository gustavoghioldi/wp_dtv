...
class Su_Shortcodes {
	...
	public static function tabs( $atts = null, $content = null ) {
		...
		if ( is_array( self::$tabs ) ) {
			...
			$cl = ( $atts['vertical'] === 'yes' ) ? '' : ' title';
			$return = '<div class="su-tabs'.$cl.' su-tabs-style-' . $atts['style'] . $atts['vertical'] . su_ecssc( $atts ) . '" data-active="' . (string) $atts['active'] . '"><div class="su-tabs-nav">' . implode( '', $tabs ) . '</div><div class="su-tabs-panes">' . implode( "\n", $panes ) . '</div></div>';
		}
		// Reset tabs
		self::$tabs = array();
		self::$tab_count = 0;
	...
	public static function button( $atts = null, $content = null ) {
		...
		if ( $atts['radius'] == 'auto' ) $radius = round( $atts['size'] - 3 ) . 'px';
	...
	public static function service( $atts = null, $content = null ) {
		$atts = shortcode_atts( array(
				...
				'icon'        => caps_su_url( 'assets/images/service.png', SU_PLUGIN_FILE ),
				'icon_color'  => '#333',
				'size'        => 32,
				'class'       => ''
			), $atts, 'service' );
	...
	public static function youtube( $atts = null, $content = null ) {
		...
		$return[] = '<i'.$frame.' width="' . $atts['width'] . '" height="' . $atts['height'] . '" src="http://www.youtube.com/embed/' . $id . $autoplay . '" frameborder="0" allowfullscreen="true"></i'.$frame.'>';
		$return[] = '</div>';
	...
	public static function youtube_advanced( $atts = null, $content = null ) {
		...
		// Create player
		$return[] = '<div class="su-youtube su-responsive-media-' . $atts['responsive'] . su_ecssc( $atts ) . '">';
		$return[] = '<i'.$frame.' width="' . $atts['width'] . '" height="' . $atts['height'] . '" src="' . $protocol . '://www.youtube.com/embed/' . $id . '?' . $params . '" frameborder="0" allowfullscreen="true"></i'.$frame.'>';
		$return[] = '</div>';
		su_query_asset( 'css', 'su-media-shortcodes' );
		// Return result
		return implode( '', $return );
	...
	public static function vimeo( $atts = null, $content = null ) {
		...
		$atts['url'] = su_scattr( $atts['url'] );
		$id = ( preg_match( '~(?:<i'.$frame.' [^>]*src=")?(?:https?:\/\/(?:[\w]+\.)*vimeo\.com(?:[\/\w]*\/videos?)?\/([0-9]+)[^\s]*)"?(?:[^>]*></i'.$frame.'>)?(?:<p>.*</p>)?~ix', $atts['url'], $match ) ) ? $match[1] : false;
		// Check that url is specified
		...
		$return[] = '<i'.$frame.' width="' . $atts['width'] . '" height="' . $atts['height'] .
			'" src="http://player.vimeo.com/video/' . $id . '?title=0&amp;byline=0&amp;portrait=0&amp;color=ffffff' .
			$autoplay . '" frameborder="0" allowfullscreen="true"></i'.$frame.'>';
		$return[] = '</div>';
		su_query_asset( 'css', 'su-media-shortcodes' );
		// Return result
		return implode( '', $return );
	...
	public static function screenr( $atts = null, $content = null ) {
		...
		$atts['url'] = su_scattr( $atts['url'] );
		$id = ( preg_match( '~(?:<i'.$frame.' [^>]*src=")?(?:https?:\/\/(?:[\w]+\.)*screenr\.com(?:[\/\w]*\/videos?)?\/([a-zA-Z0-9]+)[^\s]*)"?(?:[^>]*></i'.$frame.'>)?(?:<p>.*</p>)?~ix', $atts['url'], $match ) ) ? $match[1] : false;
		// Check that url is specified
		...
		$return[] = '<i'.$frame.' width="' . $atts['width'] . '" height="' . $atts['height'] . '" src="http://screenr.com/embed/' . $id . '" frameborder="0" allowfullscreen="true"></i'.$frame.'>';
		$return[] = '</div>';
		su_query_asset( 'css', 'su-media-shortcodes' );
		// Return result
		return implode( '', $return );
	...
	public static function dailymotion( $atts = null, $content = null ) {
		...
		$return[] = '<i'.$frame.' width="' . $atts['width'] . '" height="' . $atts['height'] . '" src="http://www.dailymotion.com/embed/video/' . $id . '?' . implode( '&', $params ) . '" frameborder="0" allowfullscreen="true"></i'.$frame.'>';
		$return[] = '</div>';
		su_query_asset( 'css', 'su-media-shortcodes' );
		// Return result
		return implode( '', $return );
	...
	public static function document( $atts = null, $content = null ) {
		...
		su_query_asset( 'css', 'su-media-shortcodes' );
		return '<div class="su-document su-responsive-media-' . $atts['responsive'] . '"><i'.$frame.' src="http://docs.google.com/viewer?embedded=true&url=' . $atts['url'] . '" width="' . $atts['width'] . '" height="' . $atts['height'] . '" class="su-document' . su_ecssc( $atts ) . '"></i'.$frame.'></div>';
	}
	...
	public static function gmap( $atts = null, $content = null ) {
		...
		su_query_asset( 'css', 'su-media-shortcodes' );
		return '<div class="su-gmap su-responsive-media-' . $atts['responsive'] . su_ecssc( $atts ) . '"><i'.$frame.' width="' . $atts['width'] . '" height="' . $atts['height'] . '" src="http://maps.google.com/maps?q=' . urlencode( su_scattr( $atts['address'] ) ) . '&amp;output=embed"></i'.$frame.'></div>';
	}
	...
	public static function carousel( $atts = null, $content = null ) {
		...
		if ( count( $slides ) ) {
			...
			if($atts['thumbs'] === 'yes'){
				$return .= '<div class="row"><div class="col-lg-8 col-md-8 col-sm-6 col-xs-12 gallery-with-thumb">';
			}
			...
			foreach ( (array) $slides as $slide ) {
				...
				$timage = su_image_resize( $slide['image'], 100, 100 );
				$thumbs .= '<a href="#" class="thumb" data-count="'.$count.'"><img src="' . $timage['url'] . '" alt="' . esc_attr( $slide['title'] ) . '" /></a>';
				$count ++;
			}
			// Close slides
			$return .= '</div>';
			...
			if($atts['thumbs'] === 'yes'){
				$return .= '</div><div class="col-lg-4 col-md-4 col-sm-6 col-xs-12 thumbs-group">';
				$return .= $thumbs;
				$return .= '</div></div>';
			}
	...
	public static function posts( $atts = null, $content = null ) {
		...
		if($atts['template'] == 'templates/ajax-load-more.php'){
			$data_args = maybe_serialize($args);
			return "<div id='ajax-load-more'>
			<div class='listing' data-args='".$data_args."' data-path='".admin_url( 'admin-ajax.php' )."' data-display-posts='".$posts_per_page."' data-post-type='".$post_type."' data-scroll='false' data-max-pages='' data-button-text='Older Posts' data-transition='slide'></div>
			</div>";
		}
		// Save original posts
		global $posts;
		...
		$posts->boxtype =  $atts['boxtype'];
		$posts->largebox =  $atts['largebox'];
		$posts->largeboxcolumn = $atts['largeboxcolumn'];
		$posts->largeboxcategory = $atts['largeboxcategory'];
		$posts->largeboxicon = $atts['largeboxicon'];
		$posts->smallboxcolumn = $atts['smallboxcolumn'];
		$posts->excerptlength = $atts['excerptlength'];
		$posts->smallboxtype = $atts['smallboxtype'];
		$posts->smallboxexcerpt = $atts['smallboxexcerpt'];
		$posts->smallboxcomment = $atts['smallboxcomment'];
		$posts->smallboxbg = $atts['smallboxbg'];
		
		// Buffer output
		ob_start();
		// Search for template in stylesheet directory
		if ( file_exists( get_stylesheet_directory() . '/' . $atts['template'] ) ) load_template( get_stylesheet_directory() . '/' . $atts['template'], false );
		// Search for template in theme directory
		elseif ( file_exists( get_template_directory() . '/' . $atts['template'] ) ) load_template( get_template_directory() . '/' . $atts['template'], false );
