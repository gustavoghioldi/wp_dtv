...
<div id="su-about-screen">
	...
	<div class="sunrise-inline-menu">
		...
		<a href="#" target="_blank"><?php _e( 'Documentation', 'su' ); ?></a>
	...
	<div class="su-clearfix">
		<div class="su-about-column">
        <?php $url = 'http://www.youtube.com/watch'; ?>
        <?php $url1 = 'http://www.screenr.com/BK0H'; ?>
			<h3><?php _e( 'How does it works', 'su' ); ?></h3>
            
			<a href="<?php echo $url; ?>?v=DR2c266yWEA?autoplay=1&amp;showinfo=0&amp;rel=0&amp;theme=light#" target="_blank" class="su-demo-video"><img src="<?php echo caps_su_url( 'assets/images/banners/how-it-works.jpg', SU_PLUGIN_FILE ); ?>" alt=""></a>
		</div>
		<div class="su-about-column">
			<h3><?php _e( 'More videos', 'su' ); ?></h3>
			<ul>
				<li><a href="<?php echo $url; ?>?v=IjmaXz-b55I" target="_blank"><?php _e( 'Shortcodes Ultimate Tutorial', 'su' ); ?></a></li>
				<li><a href="<?php echo $url; ?>?v=YU3Zu6C5ZfA" target="_blank"><?php _e( 'How to use special widget', 'su' ); ?></a></li>
				<li><a href="<?php echo $url1; ?>" target="_blank"><?php _e( 'How to create Carousel', 'su' ); ?></a></li>
				<li><a href="<?php echo $url; ?>?v=kCWyO2F7jTw" target="_blank"><?php _e( 'How to create image gallery', 'su' ); ?></a></li>
			</ul>
		</div>
	</div>
</div>
...
<div id="su-custom-css-screen">
	<div class="su-custom-css-originals">
		<p><strong><?php _e( 'You can overview the original styles to override it', THEMENAME ); ?></strong></p>
		<div class="sunrise-inline-menu">
	...
	<div class="su-custom-css-vars">
		<p><strong><?php _e( 'You can use next variables in your custom CSS', THEMENAME ); ?></strong></p>
		<code>%home_url%</code> - <?php _e( 'home url', THEMENAME ); ?><br/>
		<code>%theme_url%</code> - <?php _e( 'theme url', THEMENAME ); ?><br/>
		<code>%plugin_url%</code> - <?php _e( 'plugin url', THEMENAME ); ?>
	</div>
	...
	public static function examples( $field, $config ) {
		...
		foreach ( $examples as $group ) {
			...
			if ( isset( $group['items'] ) ) foreach ( $group['items'] as $item ) {
					$code = ( isset( $item['code'] ) ) ? $item['code'] : caps_su_url( 'inc/examples/' . $item['id'] . '.example', SU_PLUGIN_FILE );
	...
	public static function addons( $field, $config ) {
		...
		$addons = array(
			array(
				...
				'image' => caps_su_url( 'assets/images/banners/extra.png', SU_PLUGIN_FILE )
			),
			array(
				'name' => __( 'Maker', 'su' ),
				...
				'image' => caps_su_url( 'assets/images/banners/maker.png', SU_PLUGIN_FILE )
			),
			array(
				'name' => __( 'Skins', 'su' ),
				...
				'image' => caps_su_url( 'assets/images/banners/skins.png', SU_PLUGIN_FILE )
			),
		);
		$plugins = array();
