...
class Su_Generator {
	...
	public static function button( $args = array() ) {
		...
		$args = wp_parse_args( $args, array(
				...
				'class'     => 'button',
				'icon'      => caps_su_url( 'assets/images/icon.png', SU_PLUGIN_FILE ),
				'echo'      => true,
				'shortcode' => false
			) );
		// Prepare icon
	...
	public static function popup() {
		...
		// Cache not found
			...
			$tools = apply_filters( 'su/generator/tools', array(
					'<a href="' . admin_url( 'themes.php?page=shortcodes-ultimate' ) . '#tab-1" style="display:none" target="_blank" title="' . __( 'Settings', 'su' ) . '">' . __( 'Plugin settings', 'su' ) . '</a>',
...
?>
				...
				<input type="hidden" name="su-generator-selected" id="su-generator-selected" value="<?php echo caps_su_url( '', SU_PLUGIN_FILE ); ?>" />
				<input type="hidden" name="su-generator-url" id="su-generator-url" value="<?php echo caps_su_url( '', SU_PLUGIN_FILE ); ?>" />
	...
	public static function settings() {
		...
		// Cache not found
			...
			if ( isset( $shortcode['note'] ) || isset( $shortcode['example'] ) ) {
				...
				if ( isset( $shortcode['example'] ) ) $return .= wpautop( '' );
				$return .= '</div></div>';
			}
			// Shortcode has atts
