...
if ( !class_exists( 'Sunrise4' ) ) {
	...
	class Sunrise4 {
		...
		public function register() {
			if ( isset( $this->config['menus'] ) && count( $this->config['menus'] ) )
				foreach ( $this->config['menus'] as $menu ) {
					add_theme_page( $menu['page_title'], $menu['menu_title'], $menu['capability'], $menu['slug'], array( &$this, 'render' ) );
				}
			if ( isset( $this->config['pages'] ) && count( $this->config['pages'] ) )
				foreach ( $this->config['pages'] as $page ) {
					add_theme_page( $page['page_title'], $page['menu_title'], $page['capability'], $page['slug'], array( &$this, 'render' ) );
				}
		}
		...
		public function add_menu( $args ) {
			...
			$args = wp_parse_args( $args, array(
					'page_title'  => __( 'Plugin Settings', THEMENAME ),
					'menu_title'  => __( 'Plugin Settings', THEMENAME ),
			...
			// Define page url
			if ( !$args['url'] ) $args['url'] = admin_url( 'themes.php?page=' . $args['slug'] );
		...
		public function assets() {
			// Register styles
			wp_register_style(  'sunrise', caps_su_url( $this->config['css'], $this->config['file'] ) . '/sunrise.css', false, $this->config['version'], 'all' );
			// Register scripts
			wp_register_script( 'form',    caps_su_url( $this->config['js'],  $this->config['file'] ) . '/form.js', array( 'jquery' ), $this->config['version'], true );
			wp_register_script( 'sunrise', caps_su_url( $this->config['js'],  $this->config['file'] ) . '/sunrise.js', array( 'form' ), $this->config['version'], true );
			// Add some l10n to JS
			wp_localize_script( 'sunrise', 'sunrise', array(
					'media_title'  => __( 'Choose file', THEMENAME ),
					'media_insert' => __( 'Use selected file', THEMENAME )
				) );
...
if ( !class_exists( 'Sunrise4_Views' ) ) {
	...
	class Sunrise4_Views {
		...
		public static function closetab( $field, $config ) {
			...
			$return = array();
			$return[] = '</table>';
			if ( $field['actions'] ) $return[] = '<div class="sunrise-actions-bar"><input type="submit" value="' . __( 'Save changes', THEMENAME ) . '" class="sunrise-submit button-primary" /><span class="sunrise-spin"><img src="' . admin_url( 'images/wpspin_light.gif' ) . '" alt="" /> ' . __( 'Saving', THEMENAME ) . '&hellip;</span><span class="sunrise-success-tip"><img src="' . admin_url( 'images/yes.png' ) . '" alt="" /> ' . __( 'Saved', THEMENAME ) . '</span><a href="' . $_SERVER["REQUEST_URI"] . '&amp;sunrise_action=reset" class="sunrise-reset button alignright" title="' . esc_attr( __( 'This action will delete all your settings. Are you sure? This action cannot be undone!', THEMENAME ) ) . '">' . __( 'Restore default settings', THEMENAME ) . '</a></div>';
			$return[] = '</div>';
		...
		public static function textarea( $field, $config ) {
			$field = wp_parse_args( $field, array(
					'name'  => __( 'Textarea field', THEMENAME ),
					'id'    => '',
					'desc'  => '',
					'rows'  => 10,
					'cols'  => 30
		...
		public static function checkbox( $field, $config ) {
			$field = wp_parse_args( $field, array(
					'name'  => __( 'Checkbox', THEMENAME ),
					'label' => __( 'Label', THEMENAME ),
					'id'    => '',
					'desc'  => ''
				) );
		...
		public static function options_page_notices( $options, $config ) {
			...
			$msgs = apply_filters( 'sunrise/page/notices', array(
						__( 'For full functionality of this page it is reccomended to enable javascript.', THEMENAME ),
						__( 'Settings saved successfully', THEMENAME ),
						__( 'Settings reseted successfully', THEMENAME )
					) );
			...
			$output[] = self::notice( '<a href="#" target="_blank">' . $msgs[0] . '</a>.', 'error hide-if-js' );
			// Show notice
		...
		public static function options_page_panes( $options, $config ) {
			...
			foreach ( $options as $option ) {
				...
				// Show error message
				else $panes[] = call_user_func( array( $config['views_class'], 'notice' ), 'Sunrise: ' . sprintf( __( 'option type %s is not callable', THEMENAME ), '<b>' . $option['type'] . '</b>' ), 'error' );
			}
			// Return resulting markup
