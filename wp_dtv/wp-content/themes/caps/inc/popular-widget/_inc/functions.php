   ...
   */
	...
	class PopularWidgetFunctions extends WP_Widget { 
		...
		function load_admin_styles(){
			...
			wp_enqueue_style( 'popular-admin', THEMEURI.'inc/popular-widget/_css/admin.css', NULL, $this->version );
			wp_enqueue_script( 'popular-admin', THEMEURI.'inc/popular-widget/_js/admin.js', array( 'jquery', 'jquery-ui-sortable' ), $this->version, true ); 
		}
		...
		function load_scripts_styles(){
			...
			if( ! is_admin() || is_active_widget( false, false, $this->id_base, true ) ) {
				wp_enqueue_style( 'popular-widget', THEMEURI.'inc/popular-widget/_css/pop-widget.css', NULL, $this->version );
				wp_enqueue_script( 'popular-widget', THEMEURI.'inc/popular-widget/_js/pop-widget.js', array('jquery'), $this->version, true ); 
			}
		...
		*/
		function display_post_tab_content( $posts ){
			...
			foreach( $posts as $key => $post ){ 
				...
				$output .= '<div class="single-content-small">';
				if ( has_post_thumbnail( $post->ID ) ) :
				  $output .= '<a href="'. get_permalink( $post->ID ) . '" class="image-wrap">';
				  $output .= '<img class="lazy" src="' . caps_image_resize( $fullsize[0], 100, 100, true, 'c', false ) . '" alt="' . get_the_title( $post->ID ) . '">';
				  $output .= '</a>';
				  $nothumb_class = '';
				  $tlen = 34;
				else:
				 $nothumb_class = ' full-width';
					$tlen = 50;
				endif;
				$output .= '<div class="right-desc'.$nothumb_class.'">';
				$output .= '<h5><a href="'. get_permalink( $post->ID ) . '">';
				$output .= (strlen(get_the_title( $post->ID )) > $tlen )? substr(get_the_title( $post->ID ), 0, ($tlen-2)).'..' : get_the_title( $post->ID ); 
				$output .= '</a></h5>';
				$output .= get_caps_post_info($post->ID );
				$output .='</div>';
				$output .= '</div>';			 
				$output .= '</li>';
			}
