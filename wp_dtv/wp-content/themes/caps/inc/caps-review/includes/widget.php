<?php 
class caps_review_tab_widget extends WP_Widget {
	function __construct() {
		// ajax functions
		add_action('wp_ajax_caps_review_tab_widget_content', array(&$this, 'ajax_caps_review_tab_widget_content'));
		add_action('wp_ajax_nopriv_caps_review_tab_widget_content', array(&$this, 'ajax_caps_review_tab_widget_content'));
        
        // css
        add_action('wp_enqueue_scripts', array(&$this, 'caps_review_tab_register_scripts'));
        add_action('admin_enqueue_scripts', array(&$this, 'caps_review_tab_admin_scripts'));
        add_action('customize_controls_enqueue_scripts', array(&$this, 'caps_review_tab_admin_scripts'));
		
		$widget_ops = array('classname' => 'widget_caps_review_tab', 'description' => __('Display Reviews in tabbed format.', 'mts-review'));
		$control_ops = array('width' => 200, 'height' => 350);
		$this->WP_Widget('caps_review_tab_widget', __('Caps Review Widget', 'mts-review'), $widget_ops, $control_ops);
    }	
    function caps_review_tab_admin_scripts($hook) {
        wp_register_script('caps_review_tab_widget_admin', trailingslashit( CAPS_REVIEW_ASSETS ).'js/wp-review-tab-widget-admin.js', array('jquery'));  
        wp_enqueue_script('caps_review_tab_widget_admin');
    }
    function caps_review_tab_register_scripts() { 
		// JS
        wp_enqueue_script('jquery');
		wp_register_script('caps_review_tab_widget', trailingslashit( CAPS_REVIEW_ASSETS ).'js/wp-review-tab-widget.js', array('jquery'));     
		wp_localize_script( 'caps_review_tab_widget', 'caps_review_tab',         
			array( 'ajax_url' => admin_url( 'admin-ajax.php' )) 
		);        
		// CSS     
		wp_register_style('caps_review_tab_widget', trailingslashit( CAPS_REVIEW_ASSETS ).'css/wp-review-tab-widget.css', true);
    }  
	...
	function form( $instance ) {
		...
		extract($instance);
		?>
        <div class="caps_review_tab_options_form">
        <h4><?php _e('Select Tabs', 'mts-review'); ?></h4>
        
		<div class="caps_review_tab_select_tabs">
			<label class="alignleft" style="display: block; width: 50%; margin-bottom: 7px;" for="<?php echo $this->get_field_id("tabs"); ?>_toprated">
				<input type="checkbox" class="checkbox caps_review_tab_enable_toprated" id="<?php echo $this->get_field_id("tabs"); ?>_toprated" name="<?php echo $this->get_field_name("tabs"); ?>[toprated]" value="1" <?php if (isset($tabs['toprated'])) { checked( 1, $tabs['toprated'], true ); } ?> />
			...
			<label class="alignleft" style="display: block; width: 50%; margin-bottom: 7px;" for="<?php echo $this->get_field_id("tabs"); ?>_recent">
				<input type="checkbox" class="checkbox caps_review_tab_enable_recent" id="<?php echo $this->get_field_id("tabs"); ?>_recent" name="<?php echo $this->get_field_name("tabs"); ?>[recent]" value="1" <?php if (isset($tabs['recent'])) { checked( 1, $tabs['recent'], true ); } ?> />		
			...
			<label class="alignleft" style="display: block; width: 50%;" for="<?php echo $this->get_field_id("tabs"); ?>_mostvoted">
				<input type="checkbox" class="checkbox caps_review_tab_enable_mostvoted" id="<?php echo $this->get_field_id("tabs"); ?>_mostvoted" name="<?php echo $this->get_field_name("tabs"); ?>[mostvoted]" value="1" <?php if (isset($tabs['mostvoted'])) { checked( 1, $tabs['mostvoted'], true ); } ?> />
			...
			<label class="alignleft" style="display: block; width: 50%;" for="<?php echo $this->get_field_id("tabs"); ?>_custom">
				<input type="checkbox" class="checkbox caps_review_tab_enable_custom" id="<?php echo $this->get_field_id("tabs"); ?>_custom" name="<?php echo $this->get_field_name("tabs"); ?>[custom]" value="1" <?php if (isset($tabs['custom'])) { checked( 1, $tabs['custom'], true ); } ?> />
				<?php _e( 'Custom', 'mts-review'); ?>
			</label>
		</div>
        <div class="clear"></div>
        
        <div class="caps_review_tab_advanced_options">
        
        <p class="caps_review_tab_review_type">
		...
		<p class="caps_review_tab_thumbnail_size">
        ...
        <p class="caps_review_tab_custom_reviews"<?php echo (empty($tabs['custom']) ? ' style="display: none;"' : ''); ?>>
        ...
        <h4><a href="#" class="caps_review_tab_titles_header"><?php _e('Tab Titles', 'mts-review'); ?></a> | <a href="#" class="caps_review_tab_order_header"><?php _e('Tab Order', 'mts-review'); ?></a></h4>
        
        <div class="caps_review_tab_order" style="display: none;">
            
            <label class="alignleft caps_review_tab_toprated_order" for="<?php echo $this->get_field_id('tab_order'); ?>_toprated" style="width: 50%;<?php echo (empty($tabs['toprated']) ? ' display: none;' : ''); ?>">
            ...
            <label class="alignleft caps_review_tab_recent_order" for="<?php echo $this->get_field_id('tab_order'); ?>_recent" style="width: 50%;<?php echo (empty($tabs['recent']) ? ' display: none;' : ''); ?>">
            ...
            <label class="alignleft caps_review_tab_mostvoted_order" for="<?php echo $this->get_field_id('tab_order'); ?>_mostvoted" style="width: 50%;<?php echo (empty($tabs['mostvoted']) ? ' display: none;' : ''); ?>">
            ...
            <label class="alignleft caps_review_tab_custom_order" for="<?php echo $this->get_field_id('tab_order'); ?>_custom" style="width: 50%;<?php echo (empty($tabs['custom']) ? ' display: none;' : ''); ?>">
        ...
        <div class="caps_review_tab_titles" style="display: none;">
            
            <label class="alignleft caps_review_tab_toprated_title" for="<?php echo $this->get_field_id('tab_titles'); ?>_toprated" style="width: 50%;<?php echo (empty($tabs['toprated']) ? ' display: none;' : ''); ?>">
				<?php _e('Top Rated', 'mts-review'); ?>
            ...
            <label class="alignleft caps_review_tab_recent_title" for="<?php echo $this->get_field_id('tab_titles'); ?>_recent" style="width: 50%;<?php echo (empty($tabs['recent']) ? ' display: none;' : ''); ?>">
				<?php _e('Recent', 'mts-review'); ?>
            ...
            <label class="alignleft caps_review_tab_mostvoted_title" for="<?php echo $this->get_field_id('tab_titles'); ?>_mostvoted" style="width: 50%;<?php echo (empty($tabs['mostvoted']) ? ' display: none;' : ''); ?>">
				<?php _e('Most Voted', 'mts-review'); ?>
            ...
            <label class="alignleft caps_review_tab_custom_title" for="<?php echo $this->get_field_id('tab_titles'); ?>_custom" style="width: 50%;<?php echo (empty($tabs['custom']) ? ' display: none;' : ''); ?>">
				<?php _e('Custom', 'mts-review'); ?>
        ...
        </div><!-- .caps_review_tab_advanced_options -->
		</div><!-- .caps_review_tab_options_form -->
		<?php 
	}	
	...
	function widget( $args, $instance ) {	
		...
		wp_enqueue_script('caps_review_tab_widget'); 
		wp_enqueue_style( 'wp_review-style', trailingslashit( CAPS_REVIEW_ASSETS ) . 'css/wp-review.css', array(), '1.0', 'all' );
        wp_enqueue_style('caps_review_tab_widget');  
		...
		<div class="caps_review_tab_widget_content" id="<?php echo $widget_id; ?>_content">		
    ...
    function get_most_voted($limit = 20){
        global $wpdb;
        $limit = intval($limit);
    	$table_name = $wpdb->prefix . MTS_CAPS_REVIEW_DB_TABLE;
    	if (function_exists('is_multisite') && is_multisite()) {$table_name = $wpdb->base_prefix . MTS_CAPS_REVIEW_DB_TABLE;}
	...
	function ajax_caps_review_tab_widget_content() {     
		$tab = $_POST['tab'];       
		$args = $_POST['args'];  
			...
			while ($review_query->have_posts()) : $review_query->the_post(); ?>	
				<li>
					<a title="<?php the_title(); ?>" rel="nofollow" href="<?php the_permalink() ?>">		
						<div class="caps_review_tab_thumbnail caps_review_tab_thumb_<?php echo $thumb_size; ?>">	
							<?php if(has_post_thumbnail()): ?>	
								<?php 
									$_width = ($thumb_size == 'small')? 100 : 400 ;
									$_height = ($thumb_size == 'small')? 100 : 200 ;
									$_fullsize = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'full' );
									$_url = caps_image_resize( $_fullsize[0], $_width, $_height, true, 'c', false ); 
								?>
								<img src="<?php echo $_url; ?>" alt="<?php the_title(); ?>" class="wp-post-image" />		
							<?php else: ?>
								<img src="<?php echo CAPS_REVIEW_ASSETS.'images/'.$thumb_size.'thumb.png'; ?>" alt="<?php the_title(); ?>" class="wp-post-image" />		
							<?php endif; ?>
						</div>
					</a>
					<div class="title-right">
						<h5><a title="<?php the_title(); ?>" href="<?php the_permalink() ?>"><?php echo get_the_title(); ?></a></h5>
						<?php caps_review_show_total(true, 'review-total-only '.$thumb_size.'-thumb'); ?>
	                    <?php if ( $show_date ) : ?>	
							<div class="wp-review-tab-postmeta">								
								<span class="date-meta"><i class="genericon genericon-time"></i><?php the_time('M j, Y'); ?></span>		
							</div> <!--end .entry-meta--> 				
						<?php endif; ?>	
...
add_action( 'widgets_init', create_function( '', 'register_widget( "caps_review_tab_widget" );' ) );
?>