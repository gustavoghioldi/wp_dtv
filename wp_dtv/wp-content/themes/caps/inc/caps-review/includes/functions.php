...
add_filter( 'the_content', 'caps_review_inject_data' );

/* Custom review color. */
add_action( 'wp_head', 'caps_review_color_output', 20 );
...
/* Show with shortcode */
add_shortcode('wp-review', 'caps_review_get_data');
add_shortcode('wp-review-total', 'caps_review_total_shortcode');

// image sizes for the widgets
add_image_size( 'caps_review_large', 400, 200, true ); 
add_image_size( 'caps_review_small', 100, 100, true ); 
...
function caps_review_get_data() {
	...
	$total       = get_post_meta( $post->ID, 'caps_review_total', true );
	$allowUsers  = get_post_meta( $post->ID, 'caps_review_userReview', true );
	...
	if ( $type != '' && is_singular($allowed_post_types) && is_main_query() ) {
			...
			/* Review title. */
			if( $heading != '' ){
				$review .= '<h3 class="title review"><span>' . __( $heading ) . '</span></h3>';
			}
			...
			$review .= '<div id="review" class="review-wrapper ' . $class . '" >';
				...
				if ( $items ) {
					$review .= '<ul class="review-list">';
						foreach( $items as $item ) {
							...
							$item['caps_review_item_title'] = ( !empty( $item['caps_review_item_title'] ) ) ? $item['caps_review_item_title'] : '';
							$item['caps_review_item_star'] = ( !empty( $item['caps_review_item_star'] ) ) ? $item['caps_review_item_star'] : '';

							if ( 'star' == $type ) {
								$result = $item['caps_review_item_star'] * 20;
								$bestresult = '<meta itemprop="best" content="5"/>';
								$best = '5';
							} elseif( 'point' == $type ) {
								$result = $item['caps_review_item_star'] * 10;
								$bestresult = '<meta itemprop="best" content="10"/>';
								$best = '10';
							} else {
								$result = $item['caps_review_item_star'] * 100 / 100;
							...
							$review .= '<li>';
								...
								if ( 'point' == $type ) {
									$review .= '<span>' . wp_kses_post( $item['caps_review_item_title'] ) . ' - ' . $item['caps_review_item_star'] . '/'.__('10','mts-review').'</span>';
								} elseif( 'percentage' == $type ) {
									$review .= '<span>' . wp_kses_post( $item['caps_review_item_title'] ) . ' - ' . $item['caps_review_item_star'] . '%' . '</span>';
								} else {
									$review .= '<span>' . wp_kses_post( $item['caps_review_item_title'] ) . '</span>';
								}
							...
							$review .= '<div class="review-result-wrapper">';
								...
								if ( 'star' == $type ) {
									...
									$review .= '<div class="review-result" style="width:' . $result . '%;">' . $item['caps_review_item_star'] . '/'.__('10','mts-review').'</div>';
								} else {
									$review .= '<div class="review-result" style="width:' . $result . '%;">' . $item['caps_review_item_star'] . '</div>';
								}
				...
				$review .= '<div class="review-wrap">';
				/* Review description. */
				if ( $desc ) {
						$review .= '<div class="review-desc" >';
						$review .= '<p class="review-summary-title">' . __( 'Summary', 'mts-review' ) . '</p>';
			...
			/**
				...
				*/
                ...
                $review = apply_filters('caps_review_get_data', $review, $post->ID, $type, $total, $items);
                return $review;
	} else {
		return '';
	}
...
function caps_review_inject_data( $content ) {
    global $post;
	$location = get_post_meta( $post->ID, 'caps_review_location', true );
    $location = apply_filters('caps_review_location', $location, $post->ID);
    if (empty($location) || $location == 'custom') {
        return $content;
    }
    $review = caps_review_get_data();
    if ( 'bottom' == $location ) {
 ...
 * 
 */
function caps_review_show_total($echo = true, $class = 'review-total-only') {
    global $post;
    $type = get_post_meta( $post->ID, 'caps_review_type', true );
	$total = get_post_meta( $post->ID, 'caps_review_total', true );
    $review = '';
    ...
    $review = apply_filters('caps_review_show_total', $review, $post->ID, $type, $total);
...
function caps_review_total_shortcode($atts, $content) {
    if (empty($atts['class']))
        $atts['class'] = 'review-total-only review-total-shortcode';
    
    return caps_review_show_total(false, $atts['class']);
}
 ...
 * @since 1.0
 */
function caps_review_color_output() {
	global $post;

	/* Retrieve the meta box data. */
	if(is_singular()) {
        $colors = array();
		$colors['color'] = get_post_meta( $post->ID, 'caps_review_color', true );
		$colors['type']  = get_post_meta( $post->ID, 'caps_review_type', true );
		$colors['fontcolor'] = get_post_meta( $post->ID, 'caps_review_fontcolor', true );
		$colors['bgcolor1']  = get_post_meta( $post->ID, 'caps_review_bgcolor1', true );
		$colors['bgcolor2']  = get_post_meta( $post->ID, 'caps_review_bgcolor2', true );
		$colors['bordercolor']  = get_post_meta( $post->ID, 'caps_review_bordercolor', true );
		$colors['total'] = get_post_meta( $post->ID, 'caps_review_total', true );
        
        // Filter for changing colors
        // Post ID gets passed as well
        // Usage: 
        // add_filter( 'caps_review_colors', $function_to_add, 10, 2 );
        $colors = apply_filters('caps_review_colors', $colors, $post->ID);
		...
		if( $color ) {
			...
			<?php }
			?>
			.review-list li, .review-summary-title, .review-total-box{ color: <?php echo $fontcolor; ?>;}

			
			.review-list li, .review-wrapper{ background: <?php echo $bgcolor2; ?>;}
			.review-title, .review-list li{background: <?php echo $bgcolor1; ?>;}
			...
			.review-total-wrapper, .review-wrapper, .review-title, .review-list li, .review-list li:last-child, .user-review-area{border-color: <?php echo $bordercolor; ?>;}
			<?php
			if( $total == '' ){?>
