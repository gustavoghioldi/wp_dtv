...
$(document).ready(function() {
	...
	$('#add-row').on('click', function(e) {
		...
		row.find("[name='caps_review_item_star[]']").addClass('review-star');
		$.review_total();
	});
    ...
    $('#caps_review_shortcode_hint_field').hide();
    $('#caps_review_id_hint').hide();
    
	$('#caps_review_type').on( 'change', function() {
        ...
        $('#caps_review_id_hint').toggle( $(this).val() != '' );
	...
	if ( $('#caps_review_type option:selected').val() === 'star' ) {
	...
	if ( $('#caps_review_type option:selected').val() === 'point' ) {
		$('.dynamic-text').text('Points (1-10)');
		$('#wp-review-metabox-item').show();
		$('#wp-review-metabox-heading').show();
		$('#wp-review-metabox-desc').show();
	...
	if ( $('#caps_review_type option:selected').val() === 'percentage' ) {
		$('.dynamic-text').text('Percentage (1-100)');
		$('#wp-review-metabox-item').show();
		$('#wp-review-metabox-heading').show();
		$('#wp-review-metabox-desc').show();
    ...
    if ( $('#caps_review_type option:selected').val() != '' ) {
        $('#caps_review_id_hint').show();
    }
    
    $('#caps_review_location').on('change', function() {
        $('#caps_review_shortcode_hint_field').toggle($(this).val() == 'custom');
    });
    $('#caps_review_shortcode_hint').click(function() {
        $(this).select();
    });
    if ($('#caps_review_location').val() == 'custom') {
        $('#caps_review_shortcode_hint_field').show();
    }
